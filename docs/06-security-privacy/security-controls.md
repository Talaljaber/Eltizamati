...
> **ADR-0019 auth control:** Supabase alone receives and hashes passwords. Passwords and signup codes are prohibited from routes, application persistence, snapshots, profiles, metadata, and logs. Full name, contact phone, and bank name are written only after a verified session through authenticated RLS.
...
| C2 — Personal            | email, full name, contact phone, primary bank name, locale, consent records | RLS-owned profile; no logs/analytics; consent records retained per policy below                     |
...
| Authentication  | Email/password; OTP only for first-time signup verification; Supabase session/JWT authority; automatic restore               | Auth service tests, Mailpit harness, hosted owner checklist                                             |
