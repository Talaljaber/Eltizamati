...
Companion to `threat-model.md`. Owner for every control: Talal (solo dev) — the "ownership" column therefore states the *enforcement mechanism*, which matters more here than a name.
...
| Builds | dev/preview/prod EAS profiles; debug menus stripped in preview+prod; demo mode is a *feature*, not a debug flag | Build config diff in CI |
