...
**ID scheme:** `FR-<AREA>-###`. Areas: ONB onboarding, OBL obligations, PAY payments, RATE rates, CALC calculation, SIM simulation, INS insights, EDU education, SET settings, DATA data sources/providers, AUTH auth (post-MVP), NTF notifications (post-MVP).
...
| FR-ONB-004 | User chooses a data path: **Demo data** (seeded) or **Manual entry**. Connect-a-source appears as "coming soon", never as functional. | MVP |
...
| FR-PAY-004 | Duplicate-payment detection warns when a logged payment matches an existing one (same obligation, date, amount). | S |
...
| FR-SIM-004 | Card payoff simulator: minimum-only vs fixed amount vs custom → payoff duration, finance charges, total paid, savings vs minimum; warning when payment ≤ monthly finance charge. | S |
...
| ID | Requirement | Scope |
|----|-------------|-------|
| FR-INS-001 | System generates insights from rules: rate changed, installment unchanged after rate increase, residual-balance risk, payment due soon (≤7 days), payment overdue, financing completed. | MVP |
...
| FR-INS-003 | Every insight explains *why it fired* in one sentence. | MVP |
| FR-INS-004 | Insights deduplicate: same rule + same obligation + same trigger inputs fires once. | MVP |
| FR-NTF-001 | Local scheduled notifications for payment-due reminders (OS notification, content-minimized). | S |
...
| FR-AUTH-001 | Supabase email auth + session management + sign-out + password reset. | P1 |
| FR-AUTH-002 | Versioned consent records (ToS, privacy, disclaimer, per-provider access) server-side; re-consent on material change. | P1 |
| FR-AUTH-003 | Account deletion with server-side erasure workflow + audit event. | P1 |
| FR-AUTH-004 | Optional biometric app-lock (local). | P1 |
