...
- **Loses the decision on:** new language + new framework simultaneously for a solo dev on a deadline; reviewer-fluency gap for AI output; zero skill transfer from web; ecosystem knowledge restart (state mgmt, testing idioms). Flutter is the better _framework in isolation_ and the runner-up; it is not the better choice _for this team_.
