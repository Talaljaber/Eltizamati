...
Structural dependency direction (packages cannot import the app — enforced by package.json deps + dependency-cruiser); engine tests run in milliseconds without Expo; Edge Functions later import the same `domain` schemas. Obligation: keep package count at 3 unless a boundary _proves_ itself (anti-premature-abstraction).
