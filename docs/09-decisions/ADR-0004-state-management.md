...
## Context & forces
Almost all app state is **persistent domain data** (SQLite now, remote+cache P1) plus a sliver of UI/session state. Derived financial values are explicitly *not* state (PRIN-6 — engine output, persisted as runs). Anti-pattern to prevent: one enormous global store re-implementing a database in memory.
