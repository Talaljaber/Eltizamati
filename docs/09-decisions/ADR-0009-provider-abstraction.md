...
- **Thin contract + one import pipeline — chosen:** `ObligationDataProvider` interface (provider-abstraction.md), zod-validated DTOs, one `ImportService` used by demo today and CRIF later; provenance stamped at the boundary. The demo *rehearses* the real path.
