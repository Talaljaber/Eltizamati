...
- **Why not more (≥10) for the MVP/hackathon scope:** every phase through Phase 9 has one owner-domain and one exit demo; splitting any of them would add process overhead (more completion reports, more handoffs) without reducing risk. No artificial documentation-only phase is included. **Phase 10** was added after Phase 9 closed as an explicit post-MVP-gate extension (iOS device parity, remote push notifications, saved scenarios — see [PHASE-10-ios-parity-push-and-saved-scenarios.md](../10-implementation/phases/PHASE-10-ios-parity-push-and-saved-scenarios.md)), not a revision of the original 9-phase MVP scope.
...
| 8.5 | [PHASE-08.5-visual-system-and-experience-refinement](../10-implementation/phases/PHASE-08.5-visual-system-and-experience-refinement.md) | Visual-system definition (semantic tokens, shared primitives/patterns) + controlled application to five representative experiences only; excludes financial logic, provenance semantics, architecture, and new features; closes via a recorded exit review | Planned — requires owner approval |
...
- **8 → 8.5:** Phase 8.5 requires Phase 8's core scope complete; it defines the visual system and applies it to five representative experiences only (no full-app redesign, no financial-logic/provenance/architecture changes). See the [full Phase 8.5 document](../10-implementation/phases/PHASE-08.5-visual-system-and-experience-refinement.md).
- **8.5 → 9:** Phase 9 hardening and broad visual refinement are blocked until Phase 8.5's representative-experience exit review passes.
- **4 + 8 + 8.5 → 9:** hardening validates the full surface.
...
**Phase 1 → 2 → 5 → 6 → 7.** That alone yields the scripted airplane-mode demo (onboard → populated dashboard → loan → rate impact → residual → explanation → scenario → bank questions, AR+EN). Phases 3–4 (personal mode) and Phase 8's depth items are additive to the story; Phase 8.5 then establishes the visual system on representative experiences, and Phase 9 is the polish/verification gate on whatever has landed (gated on Phase 8.5's exit review).
...
| Password auth, signup OTP verification, profile provisioning, sessions, SecureStore | ADR-0019, FR-AUTH-001/006, US-016, NFR-SEC-003, T-03/T-04, SCR-AUTH-SIGNIN/SIGNUP/VERIFY                       | 4                              |
