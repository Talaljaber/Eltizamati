...
**MVP stance (ADR-0013):** the app is **local-first because it is local-only** — SQLite is the system of record; there is nothing to sync and no offline "mode" (offline *is* the mode). This deletes the entire hardest problem class (bidirectional sync, conflicts, queues) from the hackathon while the design below keeps the P1 path honest.
