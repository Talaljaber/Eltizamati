...
> **Authentication update (2026-07-15, [ADR-0019](../09-decisions/ADR-0019-password-auth-with-signup-email-verification.md)):** personal authentication uses email/password. An eight-digit email code verifies only new registrations; the app provisions the RLS-owned profile from the verified session, then continues through consent and personal-repository entry without requiring a second sign-in. Demo mode remains auth- and network-free.
