...
- `inputsSnapshot: CanonicalJsonValue`, `inputsHash: string` — canonical JSON snapshot + deterministic hash (see `packages/domain/src/services/canonical-json.ts`: stable-key-order stringify + a pure, dependency-free non-cryptographic checksum — deliberately not Node `crypto`, so the same code runs unchanged in React Native and Edge Functions per ADR-0007's portability goal). Reproducibility (INV-5): identical `canonicalStringify(inputs)` ⇒ identical `inputsHash`.
