...
| `preview` | Signed APK for demo/judges; release JS; Sentry on; dev toggles off; demo mode available as *feature* | EAS profile `preview` |
...
Cold start ≤2.5s mid-range Android (NFR-PERF-001) · scenario calc ≤300ms/360 periods (NFR-PERF-002, benchmarked in engine tests) · dashboard 20 obligations jank-free (NFR-PERF-003) · APK size sanity check (<60MB warn). Measured on the actual demo device during rehearsal — budgets exist to be *checked*, not admired.
