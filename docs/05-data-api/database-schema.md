...
| `rate_periods`     | owner only | owner may create only the initial rate during loan creation; later rates are dashboard/service-role only | service-role dashboard only                                | never (append-only; deletion happens only via account-deletion erasure) | `user_id = auth.uid()`                | database trigger + grants enforce authority            | none                     | denied                                                |
