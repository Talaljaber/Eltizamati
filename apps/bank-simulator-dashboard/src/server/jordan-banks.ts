/**
 * Banks licensed to operate in Jordan, for the Bank Rate Simulator's
 * institution picker. Duplicated from
 * apps/mobile/src/features/auth/data/jordan-banks.ts rather than imported
 * cross-app (this repo's existing precedent — see the mapper-duplication
 * note in the dashboard design doc) — keep the two lists in sync by hand if
 * one changes. Cross-checked against the Arabic Wikipedia list of banks
 * operating in Jordan; still worth a periodic refresh against the Central
 * Bank of Jordan's own register. The Central Bank of Jordan itself is the
 * regulator, not a retail bank, and is deliberately excluded.
 */
